...
struct ContractHostFuncImpl_test : public beast::unit_test::suite
{
        ...
        uint256 const& contractHash = uint256{1})
    {
        ...
        auto const nextSequence = ac.view().read(
            keylet::account(contract.id()))->getFieldU32(sfSequence);
    ...
    testInstanceParam()
    {
        ...
        contractCtx.instanceParameters.push_back(
            ParameterValueVec{STData(sfParameterValue, uint8_t{255})});

        // UINT16
        contractCtx.instanceParameters.push_back(
            ParameterValueVec{STData(sfParameterValue, uint16_t{0x1234})});

        // UINT32
        contractCtx.instanceParameters.push_back(
            ParameterValueVec{STData(sfParameterValue, uint32_t{0x12345678})});

        // UINT64
        contractCtx.instanceParameters.push_back(
            ParameterValueVec{
                STData(sfParameterValue, uint64_t{0x123456789ABCDEF0})});

        // UINT128
        uint128 val128;
        memset(&val128, 0xAB, sizeof(val128));
        contractCtx.instanceParameters.push_back(
            ParameterValueVec{STData(sfParameterValue, val128)});

        // UINT160
        contractCtx.instanceParameters.push_back(
            ParameterValueVec{
                STData(sfParameterValue, uint160{env.master.id()})});

        // UINT192
        uint192 val192;
        for (size_t i = 0; i < val192.size(); ++i)
            val192.data()[i] = static_cast<uint8_t>(i);
        contractCtx.instanceParameters.push_back(
            ParameterValueVec{STData(sfParameterValue, uint192{val192})});

        // UINT256
        uint256 val256 = beast::zero;
        val256.data()[0] = 0xFF;
        val256.data()[31] = 0xEE;
        contractCtx.instanceParameters.push_back(
            ParameterValueVec{STData(sfParameterValue, val256)});
        ...
        // Test UINT16
        {
            ...
            if (result.has_value())
            {
                ...
                BEAST_EXPECT(bytes[0] == 0x34);  // Little-endian
                BEAST_EXPECT(bytes[1] == 0x12);
            }
        }
        ...
        // Test UINT32
        {
            ...
            if (result.has_value())
            {
                ...
                BEAST_EXPECT(bytes[0] == 0x78);  // Little-endian
                BEAST_EXPECT(bytes[1] == 0x56);
                BEAST_EXPECT(bytes[2] == 0x34);
                BEAST_EXPECT(bytes[3] == 0x12);
            }
        }
        ...
        // Test UINT64
        {
            ...
            if (result.has_value())
            {
                ...
                BEAST_EXPECT(bytes[0] == 0xF0);  // Little-endian
                BEAST_EXPECT(bytes[7] == 0x12);
            }
        }
        ...
        // Test UINT192
        {
            ...
            {
                auto& bytes = result.value();
                BEAST_EXPECT(bytes.size() == 24);
                for (size_t i = 0; i < 24; ++i)
                {
                    BEAST_EXPECT(bytes[i] == static_cast<uint8_t>(i));
                }
            }
        }
        ...
        // Test UINT256
        {
            ...
            {
                auto& bytes = result.value();
                BEAST_EXPECT(bytes.size() == 32);
                BEAST_EXPECT(bytes[0] == 0xFF);
                BEAST_EXPECT(bytes[31] == 0xEE);
            }
        }
    ...
    testEmitBuiltTxn()
    {
        ...
        //         limit.add(limitSer);
        //         auto limitResult =
        //             cfs.addTxnField(txIndex, sfLimitAmount, limitSer.slice());
        ...
        //             BEAST_EXPECT(terCode != static_cast<int32_t>(tesSUCCESS));
        //         }
        //     }
        // }
